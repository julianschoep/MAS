...
vacuums-own [beliefs desire intention location dist ]
...
to update-beliefs
  ...
  ask patches[
    if pcolor = brown [
      let xint int pxcor
      let yint int pycor
      set dirt-list lput list xint yint dirt-list
      ]
   ]
   ask vacuums[
     set beliefs dirt-list
     set total_dirty length(beliefs)
   ]
...
to update-intentions
  ...
  ask vacuums[
    ...
    if desire = 1[
      ...
      let target [0 0]
      foreach beliefs[
        let xcor_patch int (item 0 ?)
        let ycor_patch int (item 1 ?)
        let cord_dist sqrt((xcor - xcor_patch) ^ 2 + (ycor - ycor_patch) ^ 2)
        if cord_dist < min_distance[
          set min_distance cord_dist
          set intention list xcor_patch ycor_patch
          set dist cord_dist
        ]
      ]
    ]
    if desire = 0[
      set intention list 0 0
    ]
  ]
...
to execute-actions
  ask vacuums[
    if desire = 1[
      ...
      [ask patch-here [set pcolor white]] ; Sitting on dirt. Clean it.
      [facexy item 0 intention item 1 intention
       fd 1]
    ]
  ]
